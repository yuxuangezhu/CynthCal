...
public extension Calendar {
  ...
  static var solar: Calendar {
    // Honor the app-level override when set, otherwise follow the system
    let preferredFirstWeekday = WeekdayPreference.shared.firstWeekday
    let current = autoupdatingCurrent
    if current.identifier == .gregorian {
      if let preferredFirstWeekday, current.firstWeekday != preferredFirstWeekday {
        var customized = current
        customized.firstWeekday = preferredFirstWeekday
        return customized
      }
    ...
    gregorian.firstWeekday = preferredFirstWeekday ?? current.firstWeekday
    return gregorian
  }
