...
final class CalendarTests: XCTestCase {
  ...
  func testWeekdayPreferenceOverride() {
    let previous = WeekdayPreference.shared.firstWeekday
    defer { WeekdayPreference.shared.firstWeekday = previous }

    // nil = follow the system
    WeekdayPreference.shared.firstWeekday = nil
    XCTAssertEqual(Calendar.solar.firstWeekday, Calendar.autoupdatingCurrent.firstWeekday)

    // Override to Sunday (1)
    WeekdayPreference.shared.firstWeekday = 1
    XCTAssertEqual(Calendar.solar.firstWeekday, 1)

    // Override to Monday (2)
    WeekdayPreference.shared.firstWeekday = 2
