...
struct DateDetailsView: View {
  ...
  private let anniversaries: [String]
  ...
  var body: some View {
    ...
    VStack(spacing: 0) {
      ...
      ForEach(0..<anniversaries.count, id: \.self) { index in
        HStack {
          Image(systemName: Icons.heartFill)
            .foregroundStyle(Color(Colors.systemOrange))
            .frame(width: Constants.dotSize * scale, height: Constants.dotSize * scale)
          Text(anniversaries[index])
        ...
        if index < anniversaries.count - 1 {
          Divider()
        }
      }
    ...
    .frame(minWidth: (events.isEmpty && anniversaries.isEmpty) ? 0 : 200)
